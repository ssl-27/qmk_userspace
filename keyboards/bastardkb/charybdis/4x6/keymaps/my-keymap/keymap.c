...
// Define your layer colors
#define HSV_BASE     167, 255, 100  // Teal
#define HSV_LOWER    220, 255, 100  // Blue
#define HSV_RAISE    0,   255, 100  // Red
#define HSV_POINTER  85,  255, 100  // Green
#define HSV_CAPSLOCK 0,   255, 255  // Bright Red
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  // ├──────────────────────────────────────────────────────┤ ├──────────────────────────────────────────────────────┤
       KC_TAB,    KC_Z,    KC_X,    KC_C,    PT_V,    KC_B,       KC_N,    KC_M, KC_COMM,  KC_DOT, PT_SLSH, KC_LALT,
  // ╰──────────────────────────────────────────────────────┤ ├──────────────────────────────────────────────────────╯
                                   KC_LGUI, KC_SPC, KC_BSPC,      RAISE,  KC_ENT,
  ...
  // ├──────────────────────────────────────────────────────┤ ├──────────────────────────────────────────────────────┤
       XXXXXXX, KC_LGUI, KC_LALT, KC_LCTL, KC_LSFT, XXXXXXX,    XXXXXXX, KC_RSFT, KC_BTN3, KC_RALT, KC_RGUI, XXXXXXX,
  // ├──────────────────────────────────────────────────────┤ ├──────────────────────────────────────────────────────┤
       XXXXXXX, XXXXXXX, DRGSCRL, SNIPING, _______, XXXXXXX,    XXXXXXX, KC_BTN2, SNIPING, KC_BTN1, _______, XXXXXXX,
  // ╰──────────────────────────────────────────────────────┤ ├──────────────────────────────────────────────────────╯
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (host_keyboard_led_state().caps_lock) {
        RGB_MATRIX_INDICATOR_SET_COLOR_ALL(RGB_RED);
        return false;
    }
    ...
    switch (get_highest_layer(layer_state)) {
        ...
        rgb_matrix_sethsv_noeeprom(HSV_POINTER);
        break;
    }
    return false;
}

// Optional: Add this if you want to handle RGB effects when layers change
layer_state_t layer_state_set_user(layer_state_t state) {
#ifdef CHARYBDIS_AUTO_SNIPING_ON_LAYER
    // Keep your existing sniping layer code
    charybdis_set_pointer_sniping_enabled(layer_state_cmp(state, CHARYBDIS_AUTO_SNIPING_ON_LAYER));
#endif

    // Force a refresh of the RGB matrix
    rgb_matrix_update_pwm_buffers();

    return state;
}

// Add this to handle Caps Lock changes
bool led_update_user(led_t led_state) {
    ...
    if (caps_state != led_state.caps_lock) {
        ...
        if (caps_state) {
            ...
            rgb_matrix_sethsv_noeeprom(HSV_CAPSLOCK);
        } else {
            rgb_matrix_mode_noeeprom(RGB_MATRIX_DEFAULT_MODE);
